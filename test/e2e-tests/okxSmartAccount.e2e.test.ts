...
import { Chain, createWalletClient, Hex, http, isAddress, publicActions, WalletClient, zeroAddress } from "viem";
import { privateKeyToAccount } from "viem/accounts";
import { polygon } from "viem/chains";
import { describe, expect, it } from "vitest";
import { LocalAccountAASigner } from "../../packages/plugins/signers/localAccountAASigner";
import { generatePrivateKey } from "viem/accounts";
import { givenConnectedProvider, givenConnectedProviderWithPaymaster } from "../utils";
...
describe("OKX Smart Account EntryPoint v7 Tests", () => {
  ...
  it("should successfully get counterfactual address", async () => {
    ...
    expect(await provider.getAddress()).toMatchInlineSnapshot(`"0xfc386Ff841DeB3879446808A90307Ca0511B8676"`);
  });

  it("should encode successfully", async () => {
    const provider = await givenConnectedProvider({ signer, chain });
    expect(await provider.encodeExecute({ to: zeroAddress, value: 1n, data: "0x" })).toMatchInlineSnapshot(
  ...
  it("should get counterfactual for undeployed account", async () => {
    const signer = LocalAccountAASigner.privateKeyToAccountSigner(generatePrivateKey());
    const provider = await givenConnectedProvider({ signer, chain });
